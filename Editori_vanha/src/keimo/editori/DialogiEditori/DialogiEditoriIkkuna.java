package keimo.editori.DialogiEditori;

import keimo.Utility.Downloaded.SpringUtilities;
import keimo.editori.CustomViestiIkkunat;
...
public class DialogiEditoriIkkuna {
    ...
    private static void uudelleenAlustaDialogiKartta() {
        List<VuoropuheDialogiPätkä> alkupVdpt = Dialogit.PitkätDialogit.vuoropuheDialogiKartta.values().stream().toList();
        editorinDialogiKartta.clear();
        for (VuoropuheDialogiPätkä vdp : alkupVdpt) {
    ...
    // private static void kopioiMuokattuDialogiKartta() {
    //     List<VuoropuheDialogiPätkä> editorinVdpt = editorinDialogiKartta.values().stream().toList();
    //     Dialogit.PitkätDialogit.vuoropuheDialogiKartta.clear();
    ...
    //             kopioidutTriggerit = Arrays.copyOf(vdp.annaTriggerit(), vdp.annaTriggerit().length);
    //         }
    //         Dialogit.PitkätDialogit.vuoropuheDialogiKartta.put(vdp.annaTunniste(), new VuoropuheDialogiPätkä(vdp.annaTunniste(), vdp.annaPituus(), kopioidutKuvatiedostot, kopioidutTekstit, kopioidutPuhujat, vdp.onkoValinta(), vdp.annaValinnanNimi(), vdp.annaValinnanOtsikko(), kopioidutValinnanVaihtoehdot, kopioidutValinnanVaihtoehtojenKohdedialogit, kopioidutTriggerit));
    //     }
    // }
    
    public static void luoDialogiEditoriIkkuna() {
        
        if (!HuoneEditoriIkkuna.tiedostoLadattu) {
            JOptionPane.showMessageDialog(ikkuna, "Avaa kst-tiedosto ensin.", "Ei valittua tiedostoa", JOptionPane.INFORMATION_MESSAGE);
        }
        else {
            uudelleenAlustaDialogiKartta();

            for (VuoropuheDialogiPätkä vdp : Dialogit.PitkätDialogit.vuoropuheDialogiKartta.values()) {
                if (vdp != null) {
                    System.out.println("ladattu: " + vdp.annaTunniste());
                }
            }
            for (VuoropuheDialogiPätkä vdp : editorinDialogiKartta.values()) {
                if (vdp != null) {
                    System.out.println("editorissa: " + vdp.annaTunniste());
                }
            }
