    ...
    private static class MySwingWorker extends SwingWorker<String, Double> {
        ...
        protected String doInBackground() throws Exception {
            ...
            try {
                for (int i = 0; i < pullonPalautuksenPituus; i++) {
                    Peli.pause = true;
                    int virhePakkaus = r.nextInt(0, 10);
                    int virheMuoto = r.nextInt(0, 10);
                    int virheKäsi = r.nextInt(0, 10);
                    int virheMerkki = r.nextInt(0, 10);
                    jatkoSyöteAnnettu = false;
                    if (virhePakkaus == 0) {
                        //pulloautomaatti.valitseKuvake(PulloautomaatinKuvake.VIRHE);
                        pulloautomaatti.tila = PulloautomaatinKuvake.VIRHE;
                        virheenTyyppi = VirheenTyyppi.PAKKAUS;
                        edistymisLabel.setText("<html><p>Poista pakkaus ja yritä uudelleen!<br>(Paina Space)</p></html>");
                        while (!jatkoSyöteAnnettu) {
                            //System.out.println(jatkoSyöteAnnettu);
                            Thread.sleep(10);
                            Peli.pause = true;
                        }
                        //pulloautomaatti.valitseKuvake(PulloautomaatinKuvake.AKTIIVINEN);
                    ...
                    else if (virheMuoto == 0) {
                        pulloautomaatti.tila = PulloautomaatinKuvake.VIRHE;
                        virheenTyyppi = VirheenTyyppi.MUOTO;
                        edistymisLabel.setText("<html><p>Palauta pakkaus alkuperäiseen muotoon!<br>(Paina X)</p></html>");
                        while (!jatkoSyöteAnnettu) {
                            //System.out.println(jatkoSyöteAnnettu);
                            Thread.sleep(10);
                            Peli.pause = true;
        ...
        protected void done() {
            try {
                ...
                worker.cancel(true);
                //pulloautomaatti.valitseKuvake(PulloautomaatinKuvake.IDLE);
