...
import keimo.seikkailupeli.objektit.kenttäkohteet.esine.Kilpi;
import keimo.seikkailupeli.objektit.kenttäkohteet.esine.Kuparilager;
import keimo.seikkailupeli.objektit.kenttäkohteet.kiintopiste.KaljaAutomaatti;
...
public class PeliKenttäMetodit {
                ...
                for (Entity entity : Peli.entityLista) {
                    if (entity instanceof NPC) {
                        ...
                        if (npc instanceof Vihollinen) {
                            ...
                            if (Peli.annaObjektiKenttä()[v.annaSijX()][v.annaSijY()] instanceof Triggeri) {
                                Triggeri trg = (Triggeri)Peli.annaObjektiKenttä()[v.annaSijX()][v.annaSijY()];
    ...
    static void tarkistaTriggerit() {
        try {
            for (KenttäKohde[] kk: Peli.annaObjektiKenttä()) {
                ...
                if (Peli.entityLista.size() > 0) {
                    for (Entity entity : Peli.entityLista) {
                        if (entity instanceof LiikkuvaObjekti) {
                            ...
                            Neliö uusiSijainti = new Neliö(Pelaaja.pelaajanKokoPx, Pelaaja.pelaajanKokoPx);
                            ...
                            if (Pelaaja.hitbox.intersects(obj.hitbox)) {
                                obj.kokeileLiikkumistaTile(Pelaaja.keimonSuunta);
                            }
                        }
                    }
                }
                    ...
                    for (Entity npc : Peli.entityLista) {
                        ...
                        if (npc instanceof Asevihu) {
                            ...
                            if (!av.onkoKukistettu()) {
                                ...
                                Piste sijainti = new Piste((int)av.hitbox.getCenterX(), (int)av.hitbox.getCenterY());
                    ...
                    if (Peli.entityLista.size() > 0) {
                        for (Entity entity : Peli.entityLista) {
                            if (entity instanceof Ammus) {
                                Ammus ammus = (Ammus)entity;
                                ammus.elinAika--;
                                if (ammus.kokeileLiikettä(ammus.annaSuunta())) {
                                    ammus.liikuta8suuntaan(ammus.annaSuunta());
                                }
                                if (ammus.elinAika <= 0) {
                                    Peli.entityLista.remove(ammus);
                                    Ammus.ammusId--;
                        ...
                        if (entity instanceof Boss) {
                            ...
                            if (boss.onkoKukistettu()){
                                if (boss.annaHurtAika() <= 0) {
                                    TavoiteLista.suoritaPääTavoite(10);
    ...
    public static void luoKalja() {
        for (int y = 0; y < Peli.annaObjektiKenttä().length; y++) {
            for (int x = 0; x < Peli.annaObjektiKenttä().length; x++) {
                if (Peli.annaObjektiKenttä()[x][y] instanceof KaljaAutomaatti) {
                    if (x > 0) {
                        if (Peli.annaObjektiKenttä()[x-1][y] == null) {
                            Peli.annaObjektiKenttä()[x-1][y] = new Kuparilager(0, 0);
                        }
                    }
                }
            }
    ...
    public static void debugLuoEntity() {
        try {
            if (Peli.entityLista != null) {
                ...
                for (Entity entity : Peli.entityLista) {
                    System.out.println("entity: " + entity.annaNimi() + ", " + entity.annaSijX() + ", " + entity.annaSijY());
                }
            }
        }
        catch (ConcurrentModificationException cme) {
