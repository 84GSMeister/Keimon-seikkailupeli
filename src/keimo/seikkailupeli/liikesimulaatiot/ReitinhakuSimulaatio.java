...
public class ReitinhakuSimulaatio {
    ...
    public static void simuloiReitinhaku(Vihollinen vihollinen) {
        if (vihollinen.reitti != null) {
            if (vihollinen.reitti.size() >= 1) {
                ...
                if (vihollinen.annaSijX() == seuraavaPiste.x && vihollinen.annaSijY() == seuraavaPiste.y) {
                    vihollinen.reitti.remove(0);
                }
                else {
                    if (vihollinen.annaSijX() > seuraavaPiste.x) {
                        vihollinen.kokeileLiikkumista(Suunta.VASEN);
                    }
                    else if (vihollinen.annaSijX() < seuraavaPiste.x) {
                        vihollinen.kokeileLiikkumista(Suunta.OIKEA);
                    }
                    if (vihollinen.annaSijY() < seuraavaPiste.y) {
                        vihollinen.kokeileLiikkumista(Suunta.ALAS);
                    }
                    else if (vihollinen.annaSijY() > seuraavaPiste.y) {
    ...
    public static class PathFindingExample {
        ...
        public static boolean voiKävellä(Maasto[][] map, Point point) {
            ...
            if (Peli.annaMaastoKenttä()[point.x][point.y] != null) {
                if (Peli.annaMaastoKenttä()[point.x][point.y].estääköLiikkumisen(null)) return false;
                else return true;
            }
            else return false;
        }
        ...
        public static void reitinHakuPelaajaaKohti(Vihollinen vihollinen) {
            ...
            Maasto[][] kartta = Peli.annaMaastoKenttä();
    
            Point alku = new Point(vihollinen.annaSijX(), vihollinen.annaSijY(), null);
