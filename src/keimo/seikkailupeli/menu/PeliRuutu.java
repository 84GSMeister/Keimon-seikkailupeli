	...
	public static void lataaHuone(int huoneenId, int pelaajanX, int pelaajanY, boolean debug) {
        try {
            if (Peli.huoneKartta.get(huoneenId) != null) {
				...
				if (!ikkuna.isFullscreen()) tinyfd_messageBox("Huonetta ei löytynyt.", "Yritettiin warpata huoneeseen " + huoneenId + ", jota ei ole olemassa.", "ok", "error", false);
				Peli.voiWarpataVasen = false;
				Peli.voiWarpataOikea = false;
				Peli.voiWarpataAlas = false;
				Peli.voiWarpataYlös = false;
			...
			if (!ikkuna.isFullscreen()) tinyfd_messageBox("Huonetta ei löytynyt.", "Ongelma ladatessa huonetta " + huoneenId + ". Tämä voi johtua viallisesta default.kst-tiedostosta.", "ok", "error", false);
            e.printStackTrace();
			Peli.voiWarpataVasen = false;
			Peli.voiWarpataOikea = false;
			Peli.voiWarpataAlas = false;
