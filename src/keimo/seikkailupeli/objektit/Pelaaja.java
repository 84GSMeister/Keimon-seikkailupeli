...
public class Pelaaja extends PeliObjekti {
    ...
    public static final Neliö hitbox = new Neliö(0, 0, 64, 64);
    ...
    public static boolean vihollisenKohdalla = false;
    public static Vihollinen vihollinenKohdalla;
    ...
    /**
     * Siirrä pelaajan hitboxia nopeuden verran (pikseleissä) valittuun suuntaan.
     ...
     */
     ...
     public static boolean kokeileLiikkumista(Suunta suunta, boolean känniLiike) {
        ...
        Neliö uusiSijainti = new Neliö(64, 64);
        try {
            switch (suunta) {
                case VASEN -> {
    ...
    public static void teleporttaaLähimpäänTurvalliseenKohtaan(int alkuX, int alkuY) {
        ...
        for (int y = alkuY; y < Peli.kentänKoko; y++) {
            for (int x = alkuX; x < Peli.kentänKoko; x++) {
                if (Peli.annaMaastoKenttä()[x][y] != null) {
                    if (Peli.annaMaastoKenttä()[x][y].estääköLiikkumisen(null)) {
                        maastoTurvallinen = false;
                    }
                    else maastoTurvallinen = true;
                }
                if (Peli.annaObjektiKenttä()[x][y] instanceof VisuaalinenObjekti) {
                    VisuaalinenObjekti vo = (VisuaalinenObjekti)Peli.annaObjektiKenttä()[x][y];
                    if (vo.onkoEste()) {
    ...
    public static void vahingoita(int määrä) {
        hp -= määrä;
        kuolemattomuusAika = 120;
        Äänet.toistaSFX("pelaaja_damage", true);
        päivitäTerveys();
    }
    ...
    public static void poistaOstoskorista() {
        if (!ostosKori.isEmpty()) {
            ostosKori.remove(ostosKori.getLast());
            Pelaaja.ostostenHintaYhteensä = 0;
            for (Esine ostettavaEsine : Pelaaja.ostosKori) {
                Pelaaja.ostostenHintaYhteensä += ostettavaEsine.annaHinta();
            }
