...
public class GrafiikkaTestiMatriisit {
    ...
    private static Renderöitävä[] asetusTekstit = new Renderöitävä[] {
        new Teksti("Skaala", Väri.white, 600, 48),
        new Teksti("Sij X", Väri.white, 600, 48),
        new Teksti("Sij Y", Väri.white, 600, 48),
        new Teksti("Rot X", Väri.white, 600, 48),
        new Teksti("Rot Y", Väri.white, 600, 48),
        new Teksti("Rot Z", Väri.white, 600, 48),
        hyväksyTekstuuri,
    };

    private static Teksti[] tilaTekstit = new Teksti[] {
        new Teksti("0", Väri.white, 800, 192),
    ...
    public static void render(Shader shader, Ikkuna window) {
        ...
        for (int i = 0; i < 1; i++) {
            ...
            StaattinenRenderöinti.renderöiKomponenttiJaSkaalaa(shader, tilaTekstit[i], window, 1f/2.5f, 1f/4f, 1, 1f/2.5f +1f/4f, offsetY, 0);
        }
