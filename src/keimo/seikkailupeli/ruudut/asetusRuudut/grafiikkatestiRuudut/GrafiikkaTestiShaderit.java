...
public class GrafiikkaTestiShaderit {
    ...
    private static Renderöitävä[] asetusTekstit = new Renderöitävä[] {
        new Teksti("Shader", Väri.white, 600, 48),
        new Teksti("Nopeus", Väri.white, 600, 48),
        hyväksyTekstuuri,
    };

    private static Teksti[] tilaTekstit = new Teksti[] {
        new Teksti("0", Väri.white, 800, 48),
        new Teksti("0.1", Väri.white, 600, 48),
        new Teksti("", Väri.white, 600, 48),
    };

    private static int shadereita = Assets.annaShaderit().size();
    private static int shaderValinta = 0;
    private static float nopeus = 0.1f;
    private static float nopeusAskel = 0.01f;
    ...
    public static void render(Shader shader, Ikkuna window) {
        ...
        if (nopeus < 0) {
            String s = df.format(nopeus);
            tilaTekstit[1].päivitäTeksti("-" + s.substring(1, s.length()));
        }
        else {
            tilaTekstit[1].päivitäTeksti("" + df.format(nopeus));
        }
    ...
    private static void renderöiValittuShader() {
        ...
        Shader shader = Assets.annaShaderit().values().stream().toList().get(shaderValinta);
        shader.bind();
