...
public class EditoriRuutuVarmistus {
    private static Teksti otsikkoTeksti = new Teksti("Siirry editoriin?", Väri.white, 720, 100, KeimoFontit.fontti_keimo_36, true);
    private static Teksti tekstiTexture = new Teksti("Varmistusteksti", Väri.white, 3000, 900);
    private static Teksti jatkaNappiTexture = new Teksti("Jatka", Väri.white, 430, 48);
    ...
    public static void renderöi(Shader shader, Ikkuna window) {
        ...
        for (int i = 0; i < vaihtoehtojenMäärä; i++) {
            ...
            StaattinenRenderöinti.renderöiKomponenttiJaSkaalaa(shader, annaValikkoTeksti(i), window, 0.5f, 0.1f, 1, offsetX, -0.5f, 0);
        }

        for (int i = 0; i < vaihtoehtojenMäärä; i++) {
            float offsetX = -0.6f + i*(1f/1.5f);
            StaattinenRenderöinti.renderöiKomponenttiJaSkaalaa(shader, annaOsoitinKuvake(i), window, 0.1f, 0.1f, 1, offsetX, -0.5f, 0);
        }
    }
