...
public class KuvanValintaIkkuna {
    ...
    private static int maxScroll = Assets.annaDialogiTekstuurit().size()/sarakkeet - rivit;

    protected static void avaaKuvanValintaIkkuna() {
        DialogiEditoriRuutu.dialogiEditorinTila = Tilat.KUVAN_VALINTA;
        kuvaVaihtoehdot.clear();
        Set<String> kuvaTiedostot = Assets.annaDialogiTekstuurit().keySet();
        for (String s : kuvaTiedostot) {
            kuvaVaihtoehdot.put(new Nappi(1/6f, 1/8f, 0, 0, Assets.annaDialogiTekstuuri(s)), s);
        }
    }
            ...
            try {
                for (Nappi n : kuvaVaihtoehdot.keySet()) {
                    if (n.hiiriSisällä(hiiriX, hiiriY)) {
                        DialogiPätkänMuokkaus.muokattavaVdp.annaKuvienTiedostoNimet()[DialogiTekstinMuokkaus.muokattavaIndeksi] = kuvaVaihtoehdot.get(n);
                        suljeKuvanValintaIkkuna();
                        break;
                    }
                }
